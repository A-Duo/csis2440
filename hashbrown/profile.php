...
<?php 
    if (array_key_exists('username', $_SESSION) && $_SESSION['username'] != NULL): ?>
        ...
        <p style="font-size:2.5ch"><strong>Account: </strong>"<?= htmlspecialchars($_SESSION['username']) ?>"</p>
        <p style="font-size:2.5ch"><strong>Login count: </strong><?= $_SESSION['login-count'] ?></p>
<?php else:
